...
src/
  ...
  commands/
    ...
    checksum-address.ts # checksum-address command implementation
...
tests/
  ...
  checksum-address.test.ts # Tests for checksum-address
...
Convert an Ethereum address to its ERC-55 checksum format using viem's `getAddress`.

```
npx wooftrade@latest checksum-address [options]
```
...
```
WOOFTRADE_OK: Address checksummed successfully
{
...
```bash
# Convert a lowercase address to checksum format
