...
> 2. **No private key?** Use `npx wooftrade@0.0.10 gen-wallet` to generate a new wallet. Save the returned private key to the skill's `.env` file:
>
>    ```bash
>    echo "WOOFTRADE_PRIVATE_KEY=0x..." >> .env
>    ```
>
>    Then verify with `npx wooftrade@0.0.10 who-am-i` — it should resolve to the expected address.
>
